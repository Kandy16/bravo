...
import _thread # to run threading function
 ...
 def _wait_for_connections(self):
     ...
     while True:
         ...
         if (request_method == 'GET') | (request_method == 'HEAD'):
             ...
             # if the client request for user data then wait untill the user inputs in the console
             if(file_requested == '/get_user_input'):
                 response_headers = self._gen_headers( 200)
                 tmp = self.userInput
                 while (tmp == self.userInput) :
                     time.sleep(1)
                 response_content = bytes(self.userInput, 'utf-8')
             else:
...
def getInputFromUser(sockObj):
    while True:
        time.sleep(2)
        print('UserInput : ',end='')
        sockObj.userInput = input()
...
# create a new thread and run it to get the input from the user
_thread.start_new_thread(getInputFromUser, (s,))
